...
def run_experiment(args):
    ...
    for stage_name, model_path in stages:
        ...
        for i, sample in enumerate(tqdm(dataset)):
            image = sample["image"]
            prompt = (
                sample["question"] if "question" in sample else "What is in this image?"
            )
        ...
        print(
            f"✅ {stage_name} Results: Avg Latency={avg_lat:.2f}ms, Avg Accuracy={avg_acc:.2%}"
        )
    ...
    if results:
        ...
        metrics = {
            "Latency (ms)": [
                np.mean([r["latency"] for r in stage2_res]),
                np.mean([r["latency"] for r in stage3_res]),
            ],
            "Accuracy (Exact Match)": [
                np.mean([r["accuracy"] for r in stage2_res]),
                np.mean([r["accuracy"] for r in stage3_res]),
            ],
        }
