...
from utils_plot import save_plot, save_dual_axis_plot, save_grouped_bar
...
def regenerate_stage_comparison():
    ...
    metrics = {
        ...
        'Accuracy (Exact Match)': [stage2_res['accuracy'].mean(), stage3_res['accuracy'].mean()]
    }
    
    # Plot Latency
    save_grouped_bar(
        {'Stage 2': [metrics['Latency (ms)'][0]], 'Stage 3': [metrics['Latency (ms)'][1]]},
        ['Average'], "Stage", "Latency (ms)", "Stage Comparison - Latency", "stage_comparison_latency.png"
    )
    
    # Plot Accuracy
    save_grouped_bar(
        {'Stage 2': [metrics['Accuracy (Exact Match)'][0]], 'Stage 3': [metrics['Accuracy (Exact Match)'][1]]},
        ['Average'], "Stage", "Accuracy", "Stage Comparison - Accuracy", "stage_comparison_accuracy.png"
    )
...
def regenerate_token_budget():
    ...
    save_dual_axis_plot(
        budgets_val, lat_val, acc_val,
        "Token Budget", "Latency (ms)", "Accuracy",
